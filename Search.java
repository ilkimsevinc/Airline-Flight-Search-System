...
public class Search {
    ...
    static GregorianCalendar createDateSearch() {
    ...
    static void showDateFlight(TreeMap<Integer, Flight> allFlights, GregorianCalendar date) {
        ...
        if (dates.containsKey(date)) {
            ...
            for (Map.Entry<Integer, Flight> d : allFlights.entrySet()) {
    ...
    static void showCityDateFlights(TreeMap<Integer, Flight> allFlights, String from, GregorianCalendar date){//Dates ve from değişsin
        ...
        if (fCity.containsKey(from)) {
            System.out.println("These are the flights we found for you: ");
            for (Map.Entry<Integer, Flight> d : allFlights.entrySet()) {
                if (d.getValue().from.equals(from) && d.getValue().date.getTime().getDate() == date.getTime().getDate() && d.getValue().date.getTime().getMonth() == date.getTime().getMonth() && d.getValue().date.getTime().getYear() == date.getTime().getYear()) {
                    System.out.println(d.getValue().toString());
                }
            }
        } else {
    ...
    static String upperFirst(String str){
        if (str.length() > 1){
            ...
            if (str2.substring(0, 1).contains("İ")){
                str2 = str2.substring(0, 1).replace("İ","I") + str2.substring(1);
            }
            return str2;
        }else {
            return str;
